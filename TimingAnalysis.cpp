...
/// Author : Ali Abyaneh , Spring 2018
...
int main() {
  ...
  Graph Max;
  float skew;
  cout << "Enter Clock Skew (ns)\n";
  cin >> skew;
  float pc_to_ir = dData.DFFdelay[DFF_Model][pc].maxDelay + dData.maxDelay[NOR_Model][ins_mem] + dData.DFFdelay[DFF_Model][ir].Setup + skew;
  float pc_to_pc = dData.DFFdelay[DFF_Model][pc].maxDelay + dData.DFFdelay[DFF_Model][pc].Setup + dData.maxDelay[FA_Model][adder_4] + dData.maxDelay[MUX_Model][mux2_1] + dData.maxDelay[MUX_Model][mux2_5] + skew;

  Max.add_node(pc, {{ir, pc_to_ir}, {pc, pc_to_pc}});

  float ir_to_dec = dData.DFFdelay[DFF_Model][ir].maxDelay + dData.DFFdelay[DFF_Model][dec].Setup + std::max(std::max( dData.maxDelay[MUX_Model][mux2_3],  dData.maxDelay[MUX_Model][mux2_4]), dData.maxDelay[MUX_Model][mux2_2]) + skew;
  float ir_to_pc =  dData.maxDelay[MUX_Model][mux2_5] + dData.DFFdelay[DFF_Model][ir].maxDelay + dData.DFFdelay[DFF_Model][pc].Setup + std::max( dData.maxDelay[FA_Model][adder_4] + dData.maxDelay[MUX_Model][mux2_1] + dData.maxDelay[MUX_Model][mux2_5], dData.maxDelay[MUX_Model][mux2_1] + dData.maxDelay[MUX_Model][mux2_5]) + skew;
  Max.add_node(ir, {{dec, ir_to_dec}, {pc, ir_to_pc}});
  Max.add_node(dec, {});
  float b_to_dec = dData.DFFdelay[DFF_Model][bank_selection].maxDelay + dData.DFFdelay[DFF_Model][dec].Setup + std::max(dData.maxDelay[MUX_Model][mux2_3], dData.maxDelay[MUX_Model][mux2_4]) + skew;
  ...
  Graph Min;
  float pc_to_ir_min = dData.DFFdelay[DFF_Model][pc].minDelay + dData.minDelay[NOR_Model][ins_mem] - dData.DFFdelay[DFF_Model][ir].Hold - skew;
  float pc_to_pc_min = dData.DFFdelay[DFF_Model][pc].minDelay - dData.DFFdelay[DFF_Model][pc].Hold + dData.minDelay[HA_Model][add_75] + dData.minDelay[MUX_Model][mux2_1] - skew;

  Min.add_node(pc, {{ir, pc_to_ir_min}, {pc, pc_to_pc_min}});

  float ir_to_dec_min = dData.DFFdelay[DFF_Model][ir].minDelay - dData.DFFdelay[DFF_Model][dec].Hold + std::min(std::min( dData.minDelay[MUX_Model][mux2_3],  dData.minDelay[MUX_Model][mux2_4]), dData.minDelay[MUX_Model][mux2_2]) - skew;
  float ir_to_pc_min =  dData.minDelay[MUX_Model][mux2_5] + dData.DFFdelay[DFF_Model][ir].minDelay - dData.DFFdelay[DFF_Model][pc].Hold + std::min( dData.minDelay[FA_Model][adder_4] + dData.minDelay[MUX_Model][mux2_1] + dData.minDelay[MUX_Model][mux2_5], dData.minDelay[MUX_Model][mux2_1] + dData.minDelay[MUX_Model][mux2_5]) - skew;
  Min.add_node(ir, {{dec, ir_to_dec_min}, {pc, ir_to_pc_min}});
  Min.add_node(dec, {});
  float b_to_dec_min = dData.DFFdelay[DFF_Model][bank_selection].minDelay - dData.DFFdelay[DFF_Model][dec].Hold + std::min(dData.minDelay[MUX_Model][mux2_3], dData.minDelay[MUX_Model][mux2_4]) - skew;
  ...
  // Bonous Part   ///
  float late;
  cout << "\nEnter Mux 2_8 Late time (ns) : ";
  cin >> late;
  if(PC_ShortPaths[pc] > late)
      cout << "\nNO timing violation occured in PC-PC path \n";
  else
      cout << "\ntiming violation occured in PC-PC path \n" << "minimum slack must be > " << late << ", but it is " << PC_ShortPaths[pc]
           << "  --> " << PC_ShortPaths[pc] - late  << endl;

  if(IR_ShortPaths[pc] > late)
      cout << "\nNO timing violation occured in IR-PC path \n";
  else
      cout << "\ntiming violation occured in IR-PC path \n" << "minimum slack must be > " << late << ", but it is " << IR_ShortPaths[pc]
           << "  --> " << IR_ShortPaths[pc] - late  << endl;
  return 0;
}
